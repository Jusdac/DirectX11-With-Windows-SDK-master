...
void GameApp::UpdateScene(float dt)
{
   ...
   // m_CBuffer.proj = XMMatrixTranspose(XMMatrixPerspectiveFovLH(fov, AspectRa   tio(), 1.0f, 1000.0f));
    float left = -AspectRatio();    // 视场的左边界
    float right = AspectRatio();    // 视场的右边界
    float bottom = -1.0f;          // 视场的下边界
    float top = 1.0f;              // 视场的上边界
    float nearPlane = 1.0f;        // 近裁剪面
    float farPlane = 1000.0f;      // 远裁剪面

    // 创建正交投影矩阵
    XMMATRIX orthoProj = XMMatrixOrthographicLH(
         right-left, top-bottom, 1.0f, farPlane);

    // 转置矩阵，因为HLSL中矩阵的行和列是相反的
    m_CBuffer.proj = XMMatrixTranspose(orthoProj);
...
{ XMFLOAT3(-1.0f, 1.0f, -1.0f), XMFLOAT4(0.0f, 0.0f, 1.0f, 1.0f) },//8
    ...
    rasterizerDesc.FillMode = D3D11_FILL_SOLID;  // �������ģʽΪʵ��
    rasterizerDesc.CullMode = D3D11_CULL_NONE;   // �رձ����޳�
    rasterizerDesc.FrontCounterClockwise = false; // �������淽��Ĭ��˳ʱ��Ϊ���棩
    rasterizerDesc.DepthClipEnable = true;       // ������Ȳü�
